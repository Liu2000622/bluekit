...
def suo5_confidence(features: dict, structural: bool):
    ...
    # 已移除「无 header 纯流量特征」弱判据：长连接/双向小包/高频/URI 复用是所有 webshell
    # 交互与正常长会话的共性，会把冰蝎/哥斯拉/正常 HTTP 大量误报为 suo5。真 suo5 客户端
    # 带默认 User-Agent，或帧可解出合法 KLV 结构，靠上面两条判据即可命中。
...
def process_pcap_to_excel(input_path, output_path, status_callback=print,
    ...
    from wsat.analyzers.legacy_plugins import Suo5Plugin
