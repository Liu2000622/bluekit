...
from wsat.core.analysis_record import looks_like_valid_plaintext
...
def _xor(data: bytes, key: bytes) -> bytes:
    ...
    用大整数按块异或替代逐字节 Python 循环，大幅加速 MB 级载荷解密。"""
    if not data:
        return b""
    k = key[:16]
    if len(k) < 16:
        return bytes(data[i] ^ key[(i + 1) & 15] for i in range(len(data)))
    # 位置 i 用 key[(i+1)&15]，等价于从 key[1] 起、循环到 key[0] 的 16 字节密钥流
    rotated = k[1:16] + k[0:1]
    full = (rotated * (len(data) // 16 + 1))[:len(data)]
    return (int.from_bytes(data, "big") ^ int.from_bytes(full, "big")).to_bytes(len(data), "big")
...
def _present(data: bytes):
    """
    解密结果的呈现：若为二进制文件载荷（Java class / gzip / 序列化 / PE / ZIP…），
    返回原始 bytes 以完整保留（含魔数），供后续落盘为对应类型文件、直接反编译/解压；
    普通文本则转为展示字符串。这样二进制载荷不再因有损转码丢失魔数。
    """
    from wsat.report.payload_extractor import is_binary_file_payload
    if is_binary_file_payload(data):
        return bytes(data)
        ...
        for cand, is_nested in _behinder_transport_candidates(body):
            ...
            if magic_len and len(cand) > magic_len:
                ...
                if inner is not None:
                    ...
                    if a is not None and _behinder_plausible(a):
                        return _present(a), "AES/ECB+magic"
...
def _godzilla_result_text(data: bytes):
    """
    哥斯拉 V3/V4 请求参数常为 key + 0x02 + 4 字节长度 + value 的序列化结构。
    能解析则转成 key=value 行；否则按原字节展示。二进制文件载荷（class 内存马 / gzip /
    序列化 / PE）保留原始字节供落盘为对应类型文件。
    """
    from wsat.report.payload_extractor import is_binary_file_payload
    if is_binary_file_payload(data):
        return bytes(data)
