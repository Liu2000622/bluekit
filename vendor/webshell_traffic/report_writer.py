...
from analysis_record import (
    ...
    DECODE_BINARY_PAYLOAD, DECODE_PARTIAL,
)
...
# write_analysis_report 执行期间指向当前报告的外落写盘器
_overflow_sink = None
...
def _cell(value):
    ...
    if len(cleaned) > _XLSX_MAX:
        rel_path = _overflow_sink.save(value) if _overflow_sink else None
        if rel_path:
...
def _payload_view(r):
    """载荷/半解码/过滤内容的完整展示：二进制载荷会先置顶可读字符串，再给完整内容。"""
...
def write_analysis_report(records, output_path, stats=None,
    ...
    _overflow_sink = _OverflowSink(output_path)
    try:
