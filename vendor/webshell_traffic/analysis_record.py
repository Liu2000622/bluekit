...
def looks_like_valid_plaintext(data, min_ratio: float = 0.75,
    ...
    if exotic_ratio(text) > max_exotic:
        return False
    # 控制字符判据：真实命令/代码/JSON 几乎无控制字符（除 \t\n\r），
    # 错误密钥解出的乱码常含散落的 \x00/\x01/\x14 等控制字节。
    control = sum(1 for ch in text if ord(ch) < 0x20 and ch not in "\t\n\r")
    if control / len(text) > 0.05:
        return False
    return readable_ratio(bytes(data)) >= min_ratio
