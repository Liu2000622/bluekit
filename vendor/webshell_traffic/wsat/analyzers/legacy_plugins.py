...
from wsat.analyzers.base import AnalysisContext, AnalyzerPlugin, DetectionResult, StreamData
...
from wsat.crypto.webshell_crypto import (
    behinder_beautify,
    behinder_decrypt,
    godzilla_decrypt,
    iter_http_messages,
)
from wsat.webshell.suo5_full_analyzer import (
    is_suo5_frames,
    iter_suo5_frames,
    reconstruct_inner,
...
def _build_txn_record(stream, txn, analyzer, req_plain, res_plain, algos, detection):
    ...
    二进制文件载荷（class/gzip/序列化/PE，解密结果为 bytes）保留原始字节到 raw_payload，
    分层判定用原始字节（魔数完整），由报告落盘为对应类型文件。
    """
    client_ip, _client_port = txn.client
    server_ip, server_port = txn.server

    # 分离二进制载荷（bytes）与文本（str）：二进制完整保留原始字节
    req_text, req_raw = ((None, bytes(req_plain)) if isinstance(req_plain, (bytes, bytearray))
                         else (req_plain, None))
    res_text, res_raw = ((None, bytes(res_plain)) if isinstance(res_plain, (bytes, bytearray))
                         else (res_plain, None))
    raw_payload = req_raw if req_raw is not None else res_raw

    # 分层判定：二进制用原始字节（魔数完整），文本用字符串
    cls_req = classify_content(req_raw) if req_raw is not None else (classify_content(req_text) if req_text else None)
    cls_res = classify_content(res_raw) if res_raw is not None else (classify_content(res_text) if res_text else None)
    cls = _merge_classes(cls_req, cls_res)
    decode_status = cls["decode_status"]
    valid = decode_status == DECODE_TEXT

    combined = "\n".join(x for x in (req_text, res_text) if x)
    # 家族匹配：二进制载荷用 latin1 宽松解码提供类名等内容标记（仅匹配，不展示）
    fam_inputs = [
        req_text if req_text is not None else (req_raw.decode("latin1", "ignore") if req_raw else None),
        res_text if res_text is not None else (res_raw.decode("latin1", "ignore") if res_raw else None),
    ]
    primary_family, candidates, fam_conf, fam_ev = _sniff_family(analyzer, fam_inputs)

    has_req = bool(req_text) or (req_raw is not None)
    has_res = bool(res_text) or (res_raw is not None)
    logical = "request+response" if (has_req and has_res) else ("request" if has_req else "response")
    ...
    preview = (make_preview(combined) if combined
               else (f"[{cls['content_type']} 二进制载荷 {len(raw_payload)} 字节，见落盘文件]"
    ...
    rec = AnalysisRecord(
        ...
        request=req_text,
        response=res_text,
        decoded_command=req_text,
        decoded_response=res_text,
        content_preview=preview,
        content_sha256=(content_sha256(raw_payload) if raw_payload is not None else content_sha256(combined)),
        ...
        payload_type=(cls["content_type"] if raw_payload is not None else None),
    )
    if not valid:
...
class GodzillaPlugin(AnalyzerPlugin):
    ...
    def detect(self, stream: StreamData):
        ...
        if b"application/json, text/javascript" in low:
            return None
        # 蚁剑 ASP 一句话（VBScript eval("Ex"&chr(101)&"cute(...) 拼 Execute）是明文脚本，
        # 非哥斯拉密文，排除避免误判为哥斯拉
        _ab = unquote(req_body.decode("utf-8", "ignore")).lower()
        if "eval(" in _ab and "cute(" in _ab:
            return None
    ...
    def analyze(self, stream: StreamData, context: AnalysisContext):
        ...
        for password in candidate_keys:
            def _dec(body, is_resp, _p=password):
                ...
                # 二进制文件载荷（bytes）原样保留；仅文本明文做美化
                if isinstance(plain, str):
                    plain = behinder_beautify(plain)
                return (plain if plain else None), algo
            records, any_valid = _decode_transactions(stream, txns, self.name, _dec, detection)
            if any_valid:
                for r in records:
                    if r.is_valid_target_flow:
