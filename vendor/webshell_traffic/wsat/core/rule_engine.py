...
from wsat.core.analysis_record import RISK_HIGH, RISK_INFO, RISK_LOW, RISK_MEDIUM


def _resource_base():
    """
    规则文件所在根目录。PyInstaller 打包后数据被解压到 sys._MEIPASS，
    须优先从那里加载；否则回退为本模块所在源码目录。
    """
    meipass = getattr(sys, "_MEIPASS", None)
    if meipass:
        return meipass
    # 本模块在 core/ 子包下，项目根是其父目录（rules/ 在项目根）
    return os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


_DEFAULT_RULES_PATH = os.path.join(_resource_base(), "rules", "risk_rules.json")
